...
#' @param title_size Numeric font size for titles. Default `18`.
...
#'   `widths` is used when `auto_width = FALSE`.
...
save_data_xlsx <- function(data, file, sheet = "Data",
  ...
  if (!file.exists(file)) {
    ...
    existing_sheets <- openxlsx::getSheetNames(file)
  }
  ...
  if (is.numeric(rc) && length(rc) == 2L) {
    rc <- list(as.integer(rc))
  } else if (is.list(rc)) {
    rc <- lapply(rc, function(x) {
      if (is.numeric(x) && length(x) == 2L) return(as.integer(x))
  ...
  title_style <- openxlsx::createStyle(
    textDecoration = "bold", fontSize = title_size, fontName = font_name
  )
  ...
  if (auto_width && identical(width_scope, "global_max")) {
    for (i in seq_along(data)) {
      ...
      if (!length(global_widths[[key]])) {
        ...
        old <- global_widths[[key]]; length(old) <- m; old[is.na(old)] <- 8.43
  ...
  for (i in seq_along(data)) {
    ...
    if (!is.na(titles[[i]])) {
      openxlsx::writeData(
        wb, sheet = sheet, x = titles[[i]],
        startRow = start_row, startCol = start_col,
        colNames = FALSE, rowNames = FALSE
      )
      openxlsx::addStyle(
        wb, sheet = sheet, style = title_style,
        rows = start_row, cols = start_col, gridExpand = FALSE, stack = TRUE
      )
    ...
    if (auto_width) {
      cw <- if (identical(width_scope, "global_max")) {
        ...
        .get_col_widths(data[[i]], include_row_names = row_names)
      }
...
#' @param title_size Numeric font size for title rows. Default `16`.
#' @param font_name,font_size,border_colour,widths Styling forwarded to [write_data()].
...
save_data_xlsx_split <- function(data, file, rc = c(1L, 1L),
  ...
  if (!file.exists(file)) {
    ...
    wb <- openxlsx::loadWorkbook(file)
    existing <- openxlsx::getSheetNames(file)
  }
  ...
  title_style <- openxlsx::createStyle(
    textDecoration = "bold",
    fontSize = as.numeric(title_size),
    fontName = font_name
  )
...
#' @param title_size Numeric font size for titles. Default `16`.
...
save_plot_xlsx <- function(plot, file, sheet = "Plots",
  ...
  # Open or create workbook; ensure target sheet exists
  if (!file.exists(file)) {
    wb <- openxlsx::createWorkbook()
    existing <- character(0)
  } else {
  ...
  # Normalize rc to list of starts
  if (is.numeric(rc) && length(rc) == 2L) {
    ...
    for (i in seq_along(plots)) {
      ...
      h_rows <- (if (!is.na(titles[[i]])) 1L else 0L) +
        ...
        as.integer(row_spacer)
      cur <- c(cur[1] + h_rows, cur[2])
    }
  } else if (is.list(rc) && length(rc) == length(plots)) {
    starts <- lapply(rc, function(x) {
      if (is.numeric(x) && length(x) == 2L) as.integer(x)
      else stop("Each `rc` element must be numeric length-2: c(row, col).", call. = FALSE)
    })
  } else {
    stop("`rc` must be c(row, col) or a list of c(row, col) of length(plot).", call. = FALSE)
  }
  ...
  # Write plots (title then plot)
  for (i in seq_along(plots)) {
    start_row <- starts[[i]][1]
    start_col <- starts[[i]][2]
    ...
    # insertPlot captures the last plot drawn on the current device
    print(plots[[i]])
    openxlsx::insertPlot(
      wb, sheet = sheet,
      startRow = start_row, startCol = start_col,
...
save_plot_xlsx_split <- function(plot, file,
  ...
  # Resolve sheet names: sheet_names > names(plot) > "Sheet i"
  if (!is.null(sheet_names)) {
    if (length(sheet_names) != n)
      stop("`sheet_names` must have the same length as `plot`.", call. = FALSE)
    sheets <- as.character(sheet_names)
  } else if (!is.null(names(plots))) {
    nm <- names(plots)
    nm[!nzchar(ifelse(is.na(nm), "", nm))] <- NA_character_
  ...
  if (!is.null(plot_titles)) {
    if (length(plot_titles) != n)
      stop("`plot_titles` must have length = length(plot) (or be NULL).", call. = FALSE)
    titles <- as.character(plot_titles)
  } else {
    titles <- rep_len(NA_character_, n)
  }
  ...
  # Add ONLY the resolved 'sheets' (do NOT use names(plot) here)
  missing <- setdiff(sheets, existing)
  for (sn in missing) {
  ...
  for (i in seq_along(plots)) {
    ...
    # insertPlot uses the last plotted object on the active device
    print(plots[[i]])
    openxlsx::insertPlot(
      wb, sheet = sheet,
      startRow = start_row, startCol = start_col,
...
save_image_xlsx <- function(image, file, sheet = "Images",
                            ...
                            image_titles = NULL, title_size = 16,
                            font_name = getOption("instead.font"),
  ...
  if (length(missing)) stop("File(s) not found: ", paste(shQuote(missing), collapse = ", "))

  # resolve titles
  if (is.null(image_titles)) {
    nm <- names(image)
    titles <- if (is.null(nm)) rep_len(NA_character_, length(image)) else
      ifelse(nzchar(nm), nm, NA_character_)
  } else {
    if (length(image_titles) != length(image))
      stop("`titles` must be NULL or have the same length as `image`.", call. = FALSE)
    titles <- as.character(image_titles)
  }

  # open/create workbook & sheet
  if (!file.exists(file)) {
    wb <- openxlsx::createWorkbook()
    existing <- character(0)
  } else {
  ...
  if (!(sheet %in% existing)) openxlsx::addWorksheet(wb, sheetName = sheet, gridLines = FALSE)
  ...
  start_row <- as.integer(rc[1L]); start_col <- as.integer(rc[2L])
...
save_image_xlsx_split <- function(image, file,
  ...
  # sheet names: sheet_names > names(image) > "Sheet i"
  if (!is.null(sheet_names)) {
    if (length(sheet_names) != n)
