...
read_rds <- function(file, refhook = NULL) {
  if (missing(file)) {
    ...
    cat(sprintf("Path: %s\n", file))
    cat(sprintf("Code: instead::read_rds(\"%s\")\n", file))
  }
...
read_xl <- function(path, sheet = NULL, range = NULL, col_names = TRUE,
                    ...
                    n_max = Inf, guess_max = getOption("instead.guess_max"),
  ...
  if (is.null(sheet)) {
    ...
    cat(sprintf("Code: instead::read_xl(\"%s\", sheet = \"%s\")\n", path, sheet))
  }
  z <- readxl::read_excel(
...
read_wb <- function(xlsx_file, sheet = NULL, start_row = 1, col_names = TRUE,
  ...
  if (is.null(sheet)) {
    ...
    no <- as.numeric(no)
    sheet <- dsheets[dsheets$no == no,]$sheet
    cat(sprintf("Code: instead::read_wb(\"%s\", sheet = \"%s\")\n", xlsx_file, sheet))
  }
