...
assert_length <- function(x) {
  ...
  rlang::abort(
    ...
    class   = c("instead_error_zero_length", "instead_error", "error"),
    object  = x_name,
    length  = length(x),
    classof = class(x)
  )
...
assert_rows <- function(df) {
  ...
  rlang::abort(
    ...
    class   = c("instead_error_no_rows", "instead_error", "error"),
    df      = df_name,
    nrows   = 0L,
    columns = names(df)
  )
...
assert_attrs <- function(obj, attrs) {
  ...
  rlang::abort(
    ...
    class      = c("instead_error_missing_attributes", "instead_error", "error"),
    object     = obj_name,
    missing    = missing,
    attributes = have
  )
